...
class OrbbecCamera:
    ...
    def __init__(self, 
                 ...
                 color_fps: int = 30,
                 depth_width: int = 640, # (修改) 默认值改为 1280
                 depth_height: int = 576, # (修改) 默认值改为 720
                 depth_fps: int = 30,
                 ...
                 record_path: Optional[str] = None): # 外参输入
        """
        初始化相机管线、配置流和对齐过滤器。
        """
        ...
        if self.color_profile is None:
            print(f"警告: 找不到指定的彩色流 (RGB, {color_width}x{color_height}@{color_fps}fps)。尝试默认流...")
    ...
    def get_frames(self) -> Tuple[Optional[obs.ColorFrame], Optional[obs.DepthFrame]]:
        ...
        for attempt in range(max_retry):
            # (修改) 增加超时时间到 200ms
            raw_frameset = self.pipeline.wait_for_frames(200) 
            if raw_frameset:
                break # 成功获取，跳出循环
        ...
        if not self.enable_alignment:
            ...
            if color_frame and depth_frame:
                ...
                print("警告: 原始帧集中缺少 Color或Depth 帧。")
                raise RuntimeError("相机获取帧失败: 帧不完整。")
    ...
    def get_images_from_frames(self, color_frame: obs.ColorFrame, depth_frame: obs.DepthFrame) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        depth_image_mm = np.frombuffer(depth_frame.get_data(), dtype=np.uint16).reshape(
